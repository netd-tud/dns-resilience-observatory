"""Configuration and logging setup for the CAIDA Spoofer fetcher."""
...
_CONFIGURED = False
CONFIG_FILE = Path(__file__).with_name("spoofer.conf")
...
def load_script_config(script_path: str | Path) -> SectionProxy:
    ...
    if not parser.read(CONFIG_FILE):
        raise FileNotFoundError(f"Missing CAIDA Spoofer config file: {CONFIG_FILE}")
...
def required_config_path(script_path: str | Path, key: str) -> Path:
    configured = Path(required_config_value(script_path, key)).expanduser()
