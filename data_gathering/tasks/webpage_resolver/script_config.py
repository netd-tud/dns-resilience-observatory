"""Per-module configuration and logging setup for webpage resolver tasks."""
...
_CONFIGURED = False
CONFIG_FILE = Path(__file__).with_name("webpage_resolver.conf")
...
def load_parser() -> ConfigParser:
    parser = ConfigParser()
    if not parser.read(CONFIG_FILE):
        raise FileNotFoundError(f"Missing webpage resolver config file: {CONFIG_FILE}")
    return parser
