"""Configuration and logging setup for IPv6 Hitlist tasks."""
...
_CONFIGURED = False
CONFIG_FILE = Path(__file__).with_name("ipv6_hitlist.conf")
...
def load_script_config(script_path: str | Path) -> SectionProxy:
    ...
    # Credentials may legitimately contain ConfigParser interpolation characters such as `%`.
    parser = ConfigParser(interpolation=None)
    if not parser.read(CONFIG_FILE):
        raise FileNotFoundError(f"Missing IPv6 Hitlist config file: {CONFIG_FILE}")
...
def required_config_int(script_path: str | Path, key: str) -> int:
    ...
    try:
        ...
        raise ValueError(f"Invalid integer config value '{key}': {value!r}") from exc


def required_config_bool(script_path: str | Path, key: str) -> bool:
    value = required_config_value(script_path, key).lower()
    if value in {"1", "true", "yes", "on"}:
        return True
    if value in {"0", "false", "no", "off"}:
        return False
    raise ValueError(f"Invalid boolean config value '{key}': {value!r}")
