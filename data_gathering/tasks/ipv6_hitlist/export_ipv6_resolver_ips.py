...
def export_latest_ipv6_resolver_ips(output_dir: Path | None = None) -> dict[str, object]:
    ...
    invalid_ip_rows = 0
    non_ipv6_rows = 0
    with lzma.open(downloaded_path, mode="rt", encoding="utf-8", newline="") as source_handle:
        reader = csv.DictReader(source_handle)
        if "saddr" not in (reader.fieldnames or []):
            raise ValueError(f"IPv6 Hitlist input is missing saddr: {downloaded_path}")
        for row in reader:
            rows += 1
    ...
    output_path = output_dir / f"resolver-ipv6-{selected.measurement_date.isoformat()}.txt"
