"""Per-module configuration and logging setup for APNIC DNSSEC tasks."""
...
_CONFIGURED = False
CONFIG_FILE = Path(__file__).with_name("apnic_dnssec.conf")
...
def load_script_config(script_path: str | Path) -> SectionProxy:
    ...
    if not parser.read(CONFIG_FILE):
        raise FileNotFoundError(f"Missing APNIC DNSSEC config file: {CONFIG_FILE}")
