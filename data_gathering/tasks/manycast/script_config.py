"""Per-module configuration and logging setup for Manycast tasks."""
...
_CONFIGURED = False
CONFIG_FILE = Path(__file__).with_name("manycast.conf")
...
def load_script_config(script_path: str | Path) -> SectionProxy:
    ...
    if not parser.read(CONFIG_FILE):
        raise FileNotFoundError(f"Missing Manycast config file: {CONFIG_FILE}")
