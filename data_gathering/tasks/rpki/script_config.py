"""Per-module configuration and logging setup for RIPEstat RPKI tasks."""
...
_CONFIGURED = False
CONFIG_FILE = Path(__file__).with_name("rpki.conf")
...
def load_script_config(script_path: str | Path) -> SectionProxy:
    ...
    if not parser.read(CONFIG_FILE):
        raise FileNotFoundError(f"Missing RIPEstat RPKI config file: {CONFIG_FILE}")
...
def required_config_float(script_path: str | Path, key: str) -> float:
    ...
    try:
        ...
        raise ValueError(f"Invalid numeric config value '{key}': {value!r}") from exc
