    ...
    if verified:
        ...
        if not dry_run:
            cursor.execute(
                ...
                SET
                    verified = TRUE,
                    total_measurements = CASE WHEN total_measurements = 0 THEN 1 ELSE total_measurements END,
                    seen_measurements = CASE WHEN seen_measurements = 0 THEN 1 ELSE seen_measurements END
                FROM resolver r
...
def import_resolvers(
    ...
    for module, report in reports.items():
        ...
        if module == "resolver" and verified:
            logger.info(
